...
app.controller('movimentoacaoCtrl', ['$scope', '$http', 'api', '$filter', function ($scope, $http, api, $filter) {
	...
	$scope.editar = function (dados) {
		if (!angular.isUndefined(dados)) {
			...
			$scope.up.dataCompra = $filter('date')($scope.up.dataCompra, 'dd/MM/yyyy');
			$scope.up.dataVenda = $filter('date')($scope.up.dataVenda, 'dd/MM/yyyy');
		}
		else {
			$scope.dados = null;
			$scope.up2 = null;
	...
	function update(form) {
		$http.put(api.url + '/movimentoacao/' + form.movimentoAcaoId, form).then(function (data) {
			console.log(data);
